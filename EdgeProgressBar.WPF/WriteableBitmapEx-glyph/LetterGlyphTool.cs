...
namespace System.Windows.Media.Imaging
{
    public static class LetterGlyphTool
    {
        ...
        public static unsafe void DrawLetterVertical(this BitmapContext context, int x0, int y0, IntRect cliprect, Color fontColor, GrayScaleLetterGlyph glyph)
        {
            if (glyph.Items == null) return;
        ...
        public static int DrawStringVertical(this WriteableBitmap bmp, int x0, int y0, IntRect cliprect, Color fontColor, Color? bgColor, GlyphFont font, string text)
        {
                ...
                foreach (char ch in text)
                {
                    ...
                    if (x0 + dx <= cliprect.Right) // - here
                    {
                        if (font.IsClearType)
                        {
                            ...
                            context.DrawLetterVertical(x0 + dx, y0 - dy, cliprect, letter);
                            dy += letter.Width;
                        }
                        else
                        {
                            ...
                            if (letter == null) continue;
                            context.DrawLetterVertical(x0 + dx, y0 - dy, cliprect, fontColor, letter);
                            dy += letter.Width;
                        }
                    }
                }
        ...
        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, IntRect cliprect, Color fontColor, PortableFontDesc typeface, string text)
        {
            ...
            return bmp.DrawString(x0, y0, cliprect, fontColor, null, font, text);
        }

        public static int DrawString(this WriteableBitmap bmp, int x0, int y0, Color fontColor, Color? bgColor, PortableFontDesc typeface, string text, bool vertical=false)
        {
            var font = GetFont(typeface);
            return bmp.DrawString(x0, y0, new IntRect(new IntPoint(0, 0), new IntSize(bmp.PixelWidth, bmp.PixelHeight)), fontColor, bgColor, font, text, vertical);
        }
